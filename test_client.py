...
from src.shared.config import CV_AGENT_HOST, CV_AGENT_PORT

BASE_URL = os.getenv("TEST_BASE_URL", f"http://{CV_AGENT_HOST}:{CV_AGENT_PORT}")
...
async def test_mcp_call():
    status, data = await request(
        ...
        json={
            "tool": "find_datasets",
            "arguments": {"task_type": "detection", "domain": "general", "keyword": "COCO"},
        },
    )
    if status == 200 and "result" in data:
        print(f"[MCP Call] find_datasets -> {str(data['result'])[:120]}...")
    else:
        print(f"[MCP Call] FAIL (HTTP {status}) — {data}")


async def test_agent_request():
    print("[Agent Request] Sending query (may take 10-60s)...")
    status, data = await request(
        "POST",
        "/api/a2a/request",
        json={"query": "Recommend SOTA models for real-time object detection on COCO"},
        timeout=120.0,
    )
    if status == 200 and "agent_name" in data:
...
async def main():
    print("=" * 50)
    print("CV Research Agent — Test Client")
